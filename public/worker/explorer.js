...
function suggestedLinks(found) {
  ...
  actions.appendChild(linkButton('Open worker console', '/worker/', 'primary'));
  if (type === 'payout') actions.appendChild(linkButton('Payouts', '/worker/#payouts', ''));
  if (type === 'job') actions.appendChild(linkButton('Active job', '/worker/#active', ''));

  return actions;
}

async function resolve(id, token) {
  const res = await fetchJson(`/api/worker/resolve?id=${encodeURIComponent(id)}`, { headers: { ...authHeader(token) } });
...
async function onFind() {
  const tokenInput = qs('#workerToken');
  const idInput = qs('#id');
  const token = String(tokenInput?.value ?? '').trim();
  const id = String(idInput?.value ?? '').trim();
  if (!token) return setStatus('Missing worker token', 'bad');
  ...
  if (!res.ok) {
    ...
    setResultNode(el('div', { class: 'pw-muted', text: 'Search failed. Check your token.' }));
    return;
  }
  if (!res.found) {
    ...
    setResultType('—');
    setRaw({ found: false });
    setResultNode(el('div', { class: 'pw-muted', text: 'No matching object found for that id (or it belongs to a different worker).' }));
    return;
  }
...
function onSaveToken() {
  const token = String(qs('#workerToken')?.value ?? '').trim();
  if (!token) return toast('Missing worker token', 'bad');
  storageSet(LS.workerToken, token);
  toast('Saved worker token', 'good');
}

function init() {
  const token = storageGet(LS.workerToken, '');
  const tokenInput = qs('#workerToken');
