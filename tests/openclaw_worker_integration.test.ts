...
`;
  ...
  it('honors PROOFWORK_REQUIRE_JOB_ID and claims only the requested job', async () => {
    ...
    // Sanity: both jobs start open.
    const a0 = await getJob(jobA);
    const b0 = await getJob(jobB);
    expect(a0?.status).toBe('open');
    expect(b0?.status).toBe('open');
    ...
    // Ensure the worker actually targeted the requested job ID.
    const submitted = run.stdout.match(/submitted[\s\S]*?jobId:\s*['"]?([^'"}\s,]+)['"]?/);
    expect(submitted?.[1], `stdout=${run.stdout}`).toBe(jobB);
